...
role. Works whatever state the admin UI is in, because it writes straight to the database.
"""
...
async def _list() -> int:
    async with AsyncSessionLocal() as db:
        ...
        for user in users:
            ...
            print(f"{user.email:40} {user.role:6} {state}")
    return 0
    ...
    async with AsyncSessionLocal() as db:
        ...
        db.add(
            AuditLog(
                ...
                details="Password reset from the server shell (app.cli.reset_password)",
            )
        )
        await db.commit()
        print(f"Password set for {user.email} ({user.role}); the account is active.")
    return 0
...
def main() -> int:
    ...
    async def run() -> int:
        try:
            ...
            code = await _reset(args.email, password, args.make_admin)
